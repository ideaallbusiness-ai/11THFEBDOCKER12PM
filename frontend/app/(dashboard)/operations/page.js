...
export default function OperationsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch ALL itineraries to find which have confirmed quotes
      const itinerariesMap = {}
      const queriesWithConfirmedQuotes = []
      
      // Check each query for confirmed quotes
        ...
        try {
          ...
          if (itinRes.ok) {
            ...
            if (Array.isArray(itinData) && itinData.length > 0) {
              ...
              if (confirmedQuote) {
                itinerariesMap[query.id] = confirmedQuote
                queriesWithConfirmedQuotes.push(query)
              }
            }
          }
        } catch (e) {
