...
import { 
  ...
  ArrowLeft, IndianRupee, Save, Download, Edit, X, MapPin, PlusCircle, CheckCircle2
} from 'lucide-react'
...
export default function ItineraryBuilderPage() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [queryRes, hotelsRes, activitiesRes, routesRes, transportsRes, packagesRes, usersRes] = await Promise.all([
  ...
  const initializeFromExisting = (itinerary, queryData) => {
    ...
    setMarkupPercent(itinerary.costs?.markupPercent || 0)
    setMarkupFixed(itinerary.costs?.markupFixed || 0)
    const loadedDiscount = itinerary.costs?.discountAmount || 0
    console.log('Loading discount from itinerary:', loadedDiscount, 'Full costs:', itinerary.costs)
    setDiscountAmount(loadedDiscount)
